...
class RealtimeACC(QWidget):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("实时曲线（仿真中…）")  # 表头去掉“ACC&利用率”
        ...
        self.ax1.set_ylabel("ACC（TFLOPS）")
        self.ax2.set_ylabel("总体利用率（%）")
        (self.line_acc,)  = self.ax1.plot([], [], linewidth=1.0, marker='o', label="平均算力占用率")
        (self.line_util,) = self.ax2.plot([], [], linewidth=1.2, linestyle='--', marker='s', color='r', label="瞬时算力占用率")

        # 底部左：三条折线（NPU0/1/2 利用率）
        self.ax3 = self.fig.add_subplot(gs[1, 0])
        self.ax3.set_xlabel("时间（秒）")
        self.ax3.set_ylabel("NPU 利用率（%）")
        self.lines_npu = []
        colors = ['tab:blue', 'tab:orange', 'tab:green']
        labels = ['NPU0 利用率', 'NPU1 利用率', 'NPU2 利用率']
        ...
        self.ax4.set_title("当前时刻：各 NPU 任务构成（堆叠，单位：槽位数）")
        self.ax4.set_ylabel("槽位（0-8）")
        self.ax4.set_ylim(0, NPU_CAPACITY)

        # 右侧整列：任务队列池（单根竖向堆叠柱）
        self.ax5 = self.fig.add_subplot(gs[:, 2])
        self.ax5.set_title("任务队列池（竖向堆叠：相对预计执行时间占比）")
        self.ax5.set_ylabel("相对预计执行时间（占比 %）")
    ...
    def _draw_stackbars(self, ax, stacks):
        ...
        ax.cla()
        ax.set_title("当前时刻：各 NPU 任务构成（堆叠，单位：槽位数）")
        ax.set_ylabel("槽位（0-8）")
        ...
        for i, comp in enumerate(stacks):
            ...
            for pid, units in comp:
                ...
                if len(labels) < MAX_QUEUE_LEGEND:
                    handles.append(rects[0]); labels.append(f"任务{pid}")
    ...
    def _draw_queue_singlebar(self, ax, segments):
        ...
        """
        ax.cla()
        ax.set_title("任务队列池（竖向堆叠：相对预计执行时间占比）")
        ax.set_ylabel("相对预计执行时间（占比 %）")
        ax.set_ylim(0, 100)
        ax.set_xticks([0])
        ax.set_xticklabels(["任务队列池"])

        if not segments:
            ax.text(0.5, 0.5, "队列为空", ha='center', va='center', transform=ax.transAxes, alpha=0.6)
            return
    ...
    def update_plot(self):
        ...
        # 窗口标题：瞬时算力 / 平均算力 / 总体利用率 / 上限 + NPU首个任务
        avg_now = y_acc[-1] if y_acc else 0.0
        self.setWindowTitle(
            f"实时曲线 | 瞬时算力≈{inst:.2f} TFLOPS | 平均算力≈{avg_now:.2f} TFLOPS | 总体利用率≈{util:.0f}% | 上限≈{cap:.2f} TFLOPS | NPU任务: [{n0}] [{n1}] [{n2}]"
        )
