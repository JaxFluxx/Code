...
from sklearn.neural_network import MLPRegressor  # 引入 MLP 模型
...
# 创建 MLPRegressor 模型，调整参数（可以根据需要调整）
...
# MLP 模型没有直接的 feature_importances_ 属性
# 但可以通过权重和偏差来间接估计重要性
weights = model.coefs_

# 由于 MLP 是多层结构，我们这里只取第一层的权重来估计重要性
first_layer_weights = weights[0]  # 第一层的权重矩阵
importance_scores = np.abs(first_layer_weights).sum(axis=1)
