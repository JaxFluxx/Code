...
for col in candidates:
    ...
    print(f"[{col}]  R²={R2:.4f}  调整后R²={R2a:.4f}  | SST={SST:.3f}  SSR={SSR:.3f}  SSE={SSE:.3f}")
    print(f"  方程： Ŷ = {m.params['const']:.4f} + {m.params[col]:.4f}×{col}")
    # 如需完整表：print(m.summary())
...
while remaining:
    ...
    if (best_aic < current_aic) and all(pvals < 0.05):
        ...
        print(f"加入 {best_col}  →  AIC={best_aic:.3f}，当前自变量：{selected}")
    else:
        break
...
print(f"\n平方和分解：SST={SST:.3f}，SSR={SSR:.3f}，SSE={SSE:.3f}")
print(f"拟合优度：R²={R2:.6f}，调整后R²={R2a:.6f}")
...
plt.xlabel("拟合值 Fitted")
plt.ylabel("残差 Residual")
plt.title("Residuals vs Fitted（最终模型）")
plt.tight_layout()
plt.savefig("resid_vs_fitted_v2.png", dpi=150)

sm.qqplot(resid, line='45')
plt.title("QQ Plot of Residuals（最终模型）")
plt.tight_layout()
plt.savefig("resid_qq_v2.png", dpi=150)

print("\n图已保存：resid_vs_fitted_v2.png, resid_qq_v2.png")
